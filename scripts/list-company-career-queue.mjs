...
export function buildCompanyCareerQueue(jobs, source, now = new Date(), seeds = []) {
  ...
  return {
    ...
    operation: "select-and-audit",
    auditRequired: true,
    completionRule:
      "Every selected company must be audited in the same run, without waiting for user confirmation, until it has a complete inventory result, a concrete needs-review blocker, or a deferred-large-catalog record.",
